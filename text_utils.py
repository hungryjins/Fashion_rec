...
def create_embeddings(txt_list: List[str], model='text-embedding-3-small') -> List[np.ndarray]:
    ...
    Args:
        txt_list (List[str]): List of texts to generate embeddings for.
        model (str, optional): Embedding model.
...
def normal_chat_completion(input_prompt: str, model: str = 'gpt-4-turbo-preview') -> dict:
    """
    Generates JSON output using OpenAI chat completion.
...
def search_similar_vector(query_feature: np.array, features: List[np.array], topk: int = 10) -> Tuple[np.array, np.array]:
    """
    Compares with given vectors and provides the index and similarity of the vector similar to query_feature.
    ...
    Returns:
        Tuple[np.array, np.array]: Index and cosine similarity of similar embedding vectors.
    """
